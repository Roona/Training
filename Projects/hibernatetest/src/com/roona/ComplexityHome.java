...
public class ComplexityHome {
	...
	public Complexity findById(int id) {
		...
		try {
			Complexity instance = (Complexity) sessionFactory.getCurrentSession().get("com.roona.Complexity", id);
			if (instance == null) {
	...
	public List<Complexity> findByExample(Complexity instance) {
		...
		try {
			List<Complexity> results = (List<Complexity>) sessionFactory.getCurrentSession()
					.createCriteria("com.roona.Complexity").add(create(instance)).list();
